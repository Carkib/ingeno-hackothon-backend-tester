  ...
  public static TestResult askingForTheTwoClosestPointFromIngenoReturnsTheTwoCorrectParkings(String endpoint) {
    ...
    try {
      ...
      if (list_of_points.size() == 2) {
        result = geojsonHelper.checkIfListOfFeaturesContainsPoint(list_of_points, CLOSE_TO_INGENO_LONG1,
            CLOSE_TO_INGENO_LAT1);
        result = result && geojsonHelper.checkIfListOfFeaturesContainsPoint(list_of_points, CLOSE_TO_INGENO_LONG2,
            CLOSE_TO_INGENO_LAT2);
      }
    } catch (Exception e) {
      result = false;
    }

    return new TestResult("Asking for the two closest point from ingeno returns the two correct parkings", result);
  }
}
